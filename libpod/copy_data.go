...
func (r *libpod.Runtime) GetContainerCopyData(name string) (*CopyData, error) {
	var data *CopyData
	var err error
	if name != "" {
		data, err = openCopyData(r.store, name)
		if os.IsNotExist(errors.Cause(err)) {
			data, err = importCopyData(r.store, name, "")
		}
	}
	if err != nil {
...
func (r *Runtime) GetImageCopyData(image string) (*CopyData, error) {
	...
	data, err := ImportCopyDataFromImage(r.store, systemContext, img.ID, "", "")
	if err != nil {
...
func (r *libpod.Runtime) ImportCopyDataFromImage(systemContext *types.SystemContext, imageID, containerName, containerID string) (*CopyData, error) {
	manifest := []byte{}
	config := []byte{}
	imageName := ""
